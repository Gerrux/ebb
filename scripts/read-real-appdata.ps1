# Copies %LOCALAPPDATA%\Ambient\<file> as seen OUTSIDE any MSIX container.
...
# an autostarted Ambient writes is invisible to them. A one-off scheduled task
...
$name = "Ambient read-real-appdata ($env:USERNAME)"
$cmd = "powershell.exe -NoProfile -WindowStyle Hidden -Command Copy-Item -LiteralPath (Join-Path `$env:LOCALAPPDATA 'Ambient\$File') -Destination '$Dest' -Force"
