...
pub fn ui(ui: &mut Ui, state: &Mutex<BarState>) {
    ...
    ui.painter().rect(
        ...
        Color32::from_rgba_unmultiplied(22, 24, 30, 150),
...
fn search_ui(ui: &mut Ui, st: &mut BarState) -> bool {
    ...
    ui.scope_builder(UiBuilder::new().max_rect(query_rect).layout(Layout::left_to_right(Align::Center)), |ui| {
        ui.add_space(4.0);
        ui.label(RichText::new("\u{E721}").font(theme::icons(16.0)).color(TEXT_DIM));
        ui.add_space(8.0);
        edit = Some(
            ui.add(
    ...
    ui.scope_builder(UiBuilder::new().max_rect(chips_rect).layout(Layout::left_to_right(Align::Center)), |ui| {
        ...
        if st.query.trim().is_empty() {
            ui.label(RichText::new("Недавние").size(12.0).color(TEXT_MUTED));
        }
        for chip in chips {
            let galley = ui.painter().layout_no_wrap(chip, FontId::proportional(12.0), TEXT_DIM);
            let (r, _) = ui.allocate_exact_size(galley.size() + vec2(14.0, 6.0), Sense::hover());
            ui.painter().rect_filled(r, CornerRadius::same(7), Color32::from_rgba_unmultiplied(96, 165, 250, 45));
            ui.painter().galley(r.min + vec2(7.0, 3.0), galley, TEXT);
        }
        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
            ...
            ui.label(RichText::new(stats).size(11.5).color(TEXT_MUTED));
        });
    });
    ...
    if n == 0 {
        ...
        ui.painter().text(list.center_top() + vec2(0.0, 40.0), Align2::CENTER_TOP, msg, FontId::proportional(14.0), TEXT_MUTED);
    }
    ...
    for part in text.split([MARK_START, MARK_END]) {
        ...
        job.append(
            ...
            TextFormat {
                ...
                color: if hi { TEXT } else { color },
                background: if hi { Color32::from_rgba_unmultiplied(96, 165, 250, 70) } else { Color32::TRANSPARENT },
                ..Default::default()
            },
        );
        hi = !hi;
...
fn hit_row(ui: &Ui, r: Rect, hit: &Hit, q: &search::Query, selected: bool, hovered: bool, action: Option<usize>, confirm_delete: bool) {
    ...
    let age = painter.text(pos2(right, r.top() + 13.0), Align2::RIGHT_TOP, age(hit.updated_at), FontId::proportional(11.5), TEXT_MUTED);
    right = age.left() - 8.0;
    if hit.archived {
        let g = painter.layout_no_wrap("архив".into(), FontId::proportional(11.0), TEXT_DIM);
        ...
        painter.galley(chip.min + vec2(5.0, 2.0), g, TEXT_DIM);
        right = chip.left() - 6.0;
    }
    if hit.pinned {
    ...
    let g1 = painter.layout_job(highlighted(&line1, 14.5, TEXT, width));
    painter.galley(pos2(text_left, r.top() + 8.0), g1, TEXT);
    if action.is_some() {
        ...
        for (i, (kind, label)) in actions(hit).iter().enumerate() {
            ...
            let fill = if on { Color32::from_rgba_unmultiplied(96, 165, 250, 110) } else { Color32::from_white_alpha(14) };
            painter.rect_filled(chip, CornerRadius::same(6), fill);
            painter.galley(chip.min + vec2(7.0, 2.5), g, if on { TEXT } else { TEXT_DIM });
            x = chip.right() + 6.0;
        }
    } else if !line2.is_empty() {
        let g2 = painter.layout_job(highlighted(&line2, 12.5, TEXT_DIM, width));
        painter.galley(pos2(text_left, r.top() + 30.0), g2, TEXT_DIM);
    }
}
